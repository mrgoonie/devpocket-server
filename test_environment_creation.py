...
try:
    ...
    from app.models.environment import EnvironmentCreate, EnvironmentTemplate, ResourceLimits
...
async def test_environment_creation():
    ...
    try:
        ...
        print(f"   Resources: CPU={env_data.resources.cpu}, Memory={env_data.resources.memory}, Storage={env_data.resources.storage}")
        ...
        updated_env = await environment_service.get_environment(environment.id, str(user.id))
        if updated_env:
...
async def verify_kubernetes_deployment():
    ...
    try:
        ...
        try:
            ...
            devpocket_namespaces = [ns.metadata.name for ns in namespaces.items if ns.metadata.name.startswith('user-')]
            ...
            for namespace in devpocket_namespaces:
                try:
                    ...
                    for deployment in deployments.items:
                        print(f"🚀 Deployment: {deployment.metadata.name} in {namespace}")
                        print(f"   Status: Ready={deployment.status.ready_replicas}/{deployment.status.replicas}")
                    ...
                    for pvc in pvcs.items:
                        if pvc.metadata.name.startswith('pvc-'):
                            ...
                            print(f"   Size: {pvc.spec.resources.requests.get('storage', 'unknown')}")
