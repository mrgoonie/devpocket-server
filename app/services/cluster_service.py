...
from app.models.cluster import (
    ClusterCreate, ClusterUpdate, ClusterInDB, ClusterResponse, 
    ClusterStatus, ClusterRegion, ClusterHealthCheck
)
...
class ClusterService:
    ...
    async def create_cluster(self, cluster_data: ClusterCreate, created_by: str) -> ClusterInDB:
        ...
        # Validate kubeconfig
        try:
            kube_config_yaml = base64.b64decode(cluster_data.kube_config).decode('utf-8')
        ...
        encrypted_config = self.cipher_suite.encrypt(cluster_data.kube_config.encode()).decode()
        ...
        logger.info("Cluster created successfully", cluster_name=cluster_data.name, region=cluster_data.region)
    ...
    async def get_cluster_by_region(self, region: ClusterRegion) -> Optional[ClusterInDB]:
    ...
    async def list_clusters(self, region: Optional[ClusterRegion] = None) -> List[ClusterResponse]:
    ...
    async def update_cluster(self, cluster_id: str, update_data: ClusterUpdate) -> Optional[ClusterInDB]:
        ...
        if 'kube_config' in update_dict:
            try:
                kube_config_yaml = base64.b64decode(update_dict['kube_config']).decode('utf-8')
                yaml.safe_load(kube_config_yaml)  # Validate YAML
                
                # Encrypt the new config
                encrypted_config = self.cipher_suite.encrypt(update_dict['kube_config'].encode()).decode()
    ...
    async def delete_cluster(self, cluster_id: str) -> bool:
        ...
        env_count = await self.db.environments.count_documents({"cluster_id": cluster_id})
        if env_count > 0:
            raise ValueError(f"Cannot delete cluster: {env_count} environments are still using it")
    ...
    async def get_decrypted_kubeconfig(self, cluster_id: str) -> Optional[str]:
        ...
        try:
            ...
            if encrypted_config:
                decrypted = self.cipher_suite.decrypt(encrypted_config.encode()).decode()
                return decrypted
        except Exception as e:
            logger.error("Failed to decrypt kubeconfig", cluster_id=cluster_id, error=str(e))
    ...
    async def check_cluster_health(self, cluster_id: str) -> ClusterHealthCheck:
        ...
        try:
            ...
            await self.db.clusters.update_one(
                {"_id": cluster_id},
                {"$set": {"status": ClusterStatus.ACTIVE, "updated_at": datetime.utcnow()}}
            )
            
        except Exception as e:
            logger.error("Cluster health check failed", cluster_id=cluster_id, error=str(e))
            health_check.error_message = str(e)
            
            # Update cluster status to error
            await self.db.clusters.update_one(
                {"_id": cluster_id},
                {"$set": {"status": ClusterStatus.INACTIVE, "updated_at": datetime.utcnow()}}
            )
