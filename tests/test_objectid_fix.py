...
class TestObjectIdFix:
    ...
    async def test_user_creation_with_objectid_validation(self, client: AsyncClient, clean_database, sample_user_data):
        ...
        assert ObjectId.is_valid(data["id"])  # Should be valid ObjectId format but as string
    
    async def test_login_with_existing_user_objectid_fix(self, client: AsyncClient, clean_database, sample_user_data):
    ...
    async def test_get_current_user_objectid_conversion(self, client: AsyncClient, authenticated_user):
