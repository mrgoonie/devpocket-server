    ...
    try:
        ...
        if existing_cluster:
            ...
            if force_update:
                ...
                try:
                    response = input("Do you want to update the existing cluster? (y/N): ").strip()
...
async def verify_cluster():
    ...
    try:
        ...
        for cluster in all_clusters:
            default_marker = " [DEFAULT]" if hasattr(cluster, 'is_default') and cluster.is_default else ""
...
async def main():
    ...
    print(f"🗄️  Database: {settings.MONGODB_URL.split('@')[-1] if '@' in settings.MONGODB_URL else settings.MONGODB_URL}")
    print()
