    ...
    async def list_collection_stats(self, collections: List[str]) -> Dict[str, Any]:
        ...
        for collection_name in collections:
            try:
                ...
                stats[collection_name] = {"count": count, "exists": count > 0}
            except Exception as e:
                stats[collection_name] = {"count": 0, "exists": False, "error": str(e)}
    ...
    async def reset_collections(self, collections: List[str]) -> Dict[str, Any]:
        ...
        for collection_name in collections:
            try:
                ...
                logger.info(
                    f"Reset collection '{collection_name}' - {count_before} documents deleted"
                )
...
def display_collections():
    ...
    print(
        f"{len(AVAILABLE_COLLECTIONS) + 1:2d}. all                 - Reset ALL collections (⚠️  VERY DESTRUCTIVE)"
    )
    print("=" * 60)
...
def get_user_selection() -> List[str]:
    ...
    while True:
        try:
            selection = input(
                "\nEnter collection numbers (comma-separated) or 'q' to quit: "
            ).strip()
...
def confirm_reset(collections: List[str], stats: Dict[str, Any]) -> bool:
    ...
    if has_critical:
        ...
        confirm2 = input(
            "❓ Type 'DELETE' in uppercase to confirm deletion of critical data: "
        )
        if confirm2 != "DELETE":
            return False
