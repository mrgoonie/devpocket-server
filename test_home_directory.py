...
async def test_home_directory():
    ...
    try:
        ...
        result = subprocess.run([
            'kubectl', '--kubeconfig', kubeconfig_path,
            'exec', '-n', namespace, pod_name, '--', 'ls', '-la', '/home'
        ], capture_output=True, text=True, env={**os.environ, 'PYTHONHTTPSVERIFY': '0'})
        ...
        result = subprocess.run([
            'kubectl', '--kubeconfig', kubeconfig_path,
            'exec', '-n', namespace, pod_name, '--', 'ls', '-la', '/home/devuser'
        ], capture_output=True, text=True, env={**os.environ, 'PYTHONHTTPSVERIFY': '0'})
        ...
        result = subprocess.run([
            'kubectl', '--kubeconfig', kubeconfig_path,
            'exec', '-n', namespace, pod_name, '--', 'ls', '-la', '/workspace'
        ], capture_output=True, text=True, env={**os.environ, 'PYTHONHTTPSVERIFY': '0'})
        ...
        result = subprocess.run([
            'kubectl', '--kubeconfig', kubeconfig_path,
            'exec', '-n', namespace, pod_name, '--', 'ls', '-la', '/home/devuser/workspace'
        ], capture_output=True, text=True, env={**os.environ, 'PYTHONHTTPSVERIFY': '0'})
        ...
        result = subprocess.run([
            'kubectl', '--kubeconfig', kubeconfig_path,
            'exec', '-n', namespace, pod_name, '--', 'cat', '/workspace/test_symlink.txt'
        ], capture_output=True, text=True, env={**os.environ, 'PYTHONHTTPSVERIFY': '0'})
        print(f"Content via symlink: {result.stdout.strip()}")
        
        result = subprocess.run([
            'kubectl', '--kubeconfig', kubeconfig_path,
            'exec', '-n', namespace, pod_name, '--', 'cat', '/home/devuser/workspace/test_direct.txt'
        ], capture_output=True, text=True, env={**os.environ, 'PYTHONHTTPSVERIFY': '0'})
        print(f"Content via direct path: {result.stdout.strip()}")
        
        result = subprocess.run([
            'kubectl', '--kubeconfig', kubeconfig_path,
            'exec', '-n', namespace, pod_name, '--', 'cat', '/home/devuser/.bashrc'
        ], capture_output=True, text=True, env={**os.environ, 'PYTHONHTTPSVERIFY': '0'})
        ...
        result = subprocess.run([
            'kubectl', '--kubeconfig', kubeconfig_path,
            'exec', '-n', namespace, pod_name, '--', 'su', '-', 'devuser', '-c', 'whoami && pwd && ls -la'
        ], capture_output=True, text=True, env={**os.environ, 'PYTHONHTTPSVERIFY': '0'})
